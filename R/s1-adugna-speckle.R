...
boxcar <- function(image, KERNEL_SIZE){
  # """
  ...
  image$addBands(output, NULL, TRUE)
}


leefilter <- function(image, KERNEL_SIZE){
  # """
  ...
  stats = (image$select(bandNames)$reduceNeighborhood(
    ...
    ,optimization= 'window'))
  meanBand = bandNames$map(function(bandName) ee$String(bandName)$cat('_mean'))
  varBand = bandNames$map(function(bandName) ee$String(bandName)$cat('_variance'))
...
gammamap <- function(image,KERNEL_SIZE){
  # """
  ...
  bandNames = image$bandNames()$remove('angle')
  #local mean
  reducers = ee$Reducer$mean()$combine(reducer2= ee$Reducer$stdDev(),
                                        sharedInputs= TRUE
  )
  stats = (image$select(bandNames)$
             reduceNeighborhood(reducer= reducers,
                                                       kernel= ee$Kernel$square(KERNEL_SIZE/2,'pixels'),
                                                       optimization= 'window'))
  ...
  meanBand = bandNames$
    map(ee_utils_pyfunc(
    function(x) {
    ee$String(x)$cat('_mean')
  }))

  stdDevBand = bandNames$
    map(ee_utils_pyfunc(
      function(x) {
        ee$String(x)$cat('_stdDev')
        }))
  ...
  q = image$
    select(bandNames)$
    expression('z**2 * (z * alpha - enl - 1)**2 + 4 * alpha * enl * b() * z',
               list( 'z'= z,  'alpha'=alpha,'enl'= enl))
  ...
  #Merge
  output = ee$ImageCollection(c(zHat,rHat,x))$sum()
  image$addBands(output, NULL, TRUE)
}



RefinedLee <- function(image){
  # """
  ...
  inner <- function(b) {
    ...
    directions = directions$addBands(
      sample_mean$select(6)$subtract(sample_mean$select(4))$gt(sample_mean$select(4)$subtract(sample_mean$select(2)))$multiply(2)
    )
    directions = directions$addBands(
      sample_mean$select(3)$subtract(sample_mean$select(4))$gt(sample_mean$select(4)$subtract(sample_mean$select(5)))$multiply(3)
    )
    directions = directions$addBands(
      sample_mean$select(0)$subtract(sample_mean$select(4))$gt(sample_mean$select(4)$subtract(sample_mean$select(8)))$multiply(4)
    )
    ...
    sigmaV = sample_stats$toArray()$arraySort()$arraySlice(0, 0, 5)$arrayReduce(ee$Reducer$mean(), list(0L))
    ...
    for (i in c(1:4)) {
      dir_mean = dir_mean$addBands(
        img$reduceNeighborhood(ee$Reducer$mean(), rect_kernel$rotate(i))$updateMask(directions$eq(2  *

                                                                                                    i  +  1))
      )
      dir_var = dir_var$addBands(
        img$reduceNeighborhood(ee$Reducer$variance(), rect_kernel$rotate(i))$updateMask(directions$eq(2  *

                                                                                                        i  +  1))
      )
      dir_mean = dir_mean$addBands(
        img$reduceNeighborhood(ee$Reducer$mean(), diag_kernel$rotate(i))$updateMask(directions$eq(2  *

                                                                                                    i  +  2))
      )
      dir_var = dir_var$addBands(
        img$reduceNeighborhood(ee$Reducer$variance(), diag_kernel$rotate(i))$updateMask(directions$eq(2  *
    ...
    result$arrayProject(list(0L))$arrayFlatten(
      ee$Reducer$sum())$
      float()
  }

  result = ee$ImageCollection(bandNames$map(ee_utils_pyfunc(inner)))$toBands()$rename(bandNames)$copyProperties(image)

  image$addBands(result, NULL, TRUE)
}




leesigma <- function(image,KERNEL_SIZE){
  # """
  ...
  stats = image$select(bandNames)$reduceNeighborhood(
                                                      ...
                                                      optimization= 'window')
  meanBand = bandNames$map(ee_utils_pyfunc(function(bandName) ee$String(bandName)$cat('_mean')))
  varBand = bandNames$map(ee_utils_pyfunc(function(bandName)  ee$String(bandName)$cat('_variance')))
  ...
  LUT = ee$Dictionary(
    list(0.5: ee$Dictionary(
    list('I1'= 0.694,'I2'= 1.385,'eta'= 0.1921)),
    list(
      "0.6"= ee$Dictionary(list('I1'= 0.630,'I2'= 1.495,'eta'= 0.2348)),
      "0.7"= ee$Dictionary(list('I1'= 0.560,'I2'= 1.627,'eta'= 0.2825)),
      "0.8"= ee$Dictionary(list('I1'= 0.480,'I2'= 1.804,'eta'= 0.3354)),
      "0.9"= ee$Dictionary(list('I1'= 0.378,'I2'= 2.094,'eta'= 0.3991)),
      "0.95"= ee$Dictionary(list('I1'= 0.302,'I2'= 2.360,'eta'= 0.4391))
    )))
...
#' Adugna MonoTemporal_Filter
#'
#' @param coll
#' @param KERNEL_SIZE
#' @param SPECKLE_FILTER
#'
#' @return
#' @export
#'
#' @examples
MonoTemporal_Filter <- function(coll,
                                KERNEL_SIZE=9,
  ...
  .filter <- function(image) {
    if (SPECKLE_FILTER == 'BOXCAR') {
      .filtered = boxcar(image, KERNEL_SIZE)
    } else if (SPECKLE_FILTER == 'LEE') {
      .filtered = leefilter(image, KERNEL_SIZE)
    }

    else if (SPECKLE_FILTER == 'GAMMA MAP') {
      .filtered = gammamap(image, KERNEL_SIZE)
    }

    else if (SPECKLE_FILTER == 'REFINED LEE') {
      .filtered = RefinedLee(image)
    }

    else if (SPECKLE_FILTER == 'LEE SIGMA') {
      .filtered = leesigma(image, KERNEL_SIZE)
    }
...
MultiTemporal_Filter <- function(coll,KERNEL_SIZE, SPECKLE_FILTER,NR_OF_IMAGES){
  # """
  #
  ...
  Quegan <- function(image){
    ...
    #
    #     """
    setresample <- function(image){
      image$resample()
    }

    get_filtered_collection <- function(image){
      # """
      ...
      check_overlap <- function(.image){
        ...
        # get all S1 frames from this date intersecting with the image bounds
        s1 = s1_coll$filterDate(.image$date(), .image$date()$advance(1, 'day'))
      ...
      ee$ImageCollection(dates$
                           map(ee_utils_pyfunc(
                             function(date){
                               s1_coll$filterDate(date, ee$Date(date)$advance(1,'day'))$toList(s1_coll$size())
                             }))$
                           flatten())
    ...
    s1 = s1$select(bands)
    meanBands = bands$map(ee_utils_pyfunc(function(bandName) ee$String(bandName)$cat('_mean')))
    ratioBands = bands$map(ee_utils_pyfunc(function(bandName) ee$String(bandName)$cat('_ratio')))
    count_img = s1$reduce(ee$Reducer$count())

    inner <- function(image){
      # """
      ...
      #       """
      if (SPECKLE_FILTER=='BOXCAR'){
        .filtered = boxcar(image, KERNEL_SIZE)$select(bands)$rename(meanBands)
      }else if(SPECKLE_FILTER=='LEE'){
        .filtered = leefilter(image, KERNEL_SIZE)$select(bands)$rename(meanBands)
      }else if (SPECKLE_FILTER=='GAMMA MAP'){
        .filtered = gammamap(image, KERNEL_SIZE)$select(bands)$rename(meanBands)
      } else if (SPECKLE_FILTER=='REFINED LEE'){
        .filtered = RefinedLee(image)$select(bands)$rename(meanBands)
      }else if (SPECKLE_FILTER=='LEE SIGMA'){
        .filtered = leesigma(image, KERNEL_SIZE)$select(bands)$rename(meanBands)
      }


      .ratio = image$select(bands)$divide(.filtered)$rename(ratioBands)
      .filtered$addBands(.ratio)
    }
